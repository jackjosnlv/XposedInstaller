...
import com.rdc.rdcsun.installer.installation.FlashCallback;
...
import com.rdc.rdcsun.installer.BuildConfig;
...
public final class InstallZipUtil {
    ...
    public static ZipCheckResult checkZip(ZipFile zip) {
        ...
        ZipEntry xposedPropEntry = zip.getEntry("system/rdcsun.prop");
        if (xposedPropEntry != null) {
            try {
                result.mXposedProp = parseXposedProp(zip.getInputStream(xposedPropEntry));
            } catch (IOException e) {
                Log.e(XposedApp.TAG, "Failed to read system/rdcsun.prop from " + zip.getName(), e);
            }
        }
